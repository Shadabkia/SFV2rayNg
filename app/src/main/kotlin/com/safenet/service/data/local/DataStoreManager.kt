...
class DataStoreManager @Inject constructor(@ApplicationContext val context: Context) {
    ...
    fun <T> getData(key: androidx.datastore.preferences.core.Preferences.Key<T>): Flow<T?> = context.dataStore.data.map { preferences ->
        preferences[key]
    }

    suspend fun <T> updateData(key: androidx.datastore.preferences.core.Preferences.Key<T>, value: T) =
